...
def horizontalDiagram (n : ℕ) := ofRowLens [n] <| (List.pairwise_singleton _ n).sortedGE
...
def horizontal_ofMultiset (M : Multiset ℕ) :
    ...
    ⟨fun i j => if h : j < M.card ∧ i = 0 then (M.sort (· ≤ ·))[j]'(by simp [h.1]) else 0,
    by
      simp only [mem_horizontalDiagram, and_imp]
      intro i j1 j2 hj hi hj2
      have hj1 : j1 < M.card := by apply lt_trans hj hj2
      simp only [hj1, hi.symm, and_self, ↓reduceDIte, hj2, ge_iff_le]
      refine List.Pairwise.rel_get_of_lt ?_ ?_
      · exact Multiset.pairwise_sort _ _
      · rwa [Fin.mk_lt_mk],
    by grind [mem_horizontalDiagram],
    by grind [mem_horizontalDiagram]⟩
...
lemma horizontal_ofMultiset_cons_largest (M : Multiset ℕ) {n : ℕ} (h : ∀ m ∈ M, m ≤ n) :
  ...
  intro j hj
  simp [horizontal_ofMultiset, DFunLike.coe, hj, Nat.le_of_lt hj, List.sort_cons' h]


lemma horizontal_ofMultiset_cons_largest_end (M : Multiset ℕ) {n : ℕ} (h : ∀ m ∈ M, m ≤ n) :
    (horizontal_ofMultiset (n ::ₘ M)) 0 M.card = n := by
  simp [DFunLike.coe, horizontal_ofMultiset, List.sort_cons' h]
...
lemma content_horizontal_ofMultiset (μ : Multiset ℕ) :
  ...
  have hγ : γ.cells = (γ.row 0) := by
    ext i
    simp [γ, horizontalDiagram, mem_ofRowLens, mem_row_iff]
    grind
  ...
  have hc : γ.rowLen 0 = μ.card := by simp [rowLen_eq_card, ← hγ, γ]
  have hc2 : μ.card = (μ.sort (· ≤ ·)).length := by simp
  simp only [hc, DFunLike.coe, horizontal_ofMultiset, Fin.val_cast, to_fun_eq_coe, Fin.is_lt,
    and_self, ↓reduceDIte]
  conv => rhs; rw [← List.ofFn_getElem (xs := μ.sort (· ≤ ·))]
  congr
...
lemma eq_horizontal_ofMultiset_content' {n : ℕ}
    ...
    T.entry = (horizontal_ofMultiset (T.content)).entry :=
  eq_horizontal_ofMultiset_content T rfl
...
theorem kostka_horizontal (μ : Multiset ℕ) :
  ...
  use ⟨horizontal_ofMultiset μ.fromCounts, by rw [Set.mem_setOf, content_horizontal_ofMultiset]⟩
  intro ⟨T, hT⟩
  rw [Subtype.mk.injEq]
