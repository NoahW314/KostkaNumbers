...
lemma kostka_ineq_not_dominate {n : ℕ} (hd : ¬ γ.rowLens ⊵ μ.sort (· ≥ ·))
...
lemma rowLens_dominates_card_iff {n : ℕ} (h : γ.card = n) :
    ([n] ⊴ γ.rowLens) ↔ γ = horizontalDiagram n := by
  by_cases hn : n = 0
  · simp [hn, h, eq_bot_iff_card_eq_zero]
  constructor
  · intro hd
...
lemma kostka_ineq_vertical {n : ℕ} (hγ : γ = verticalDiagram n)
  ...
  · rw [hγ, verticalDiagram_card] at h
    rw [hγ, verticalDiagram_rowLens, replicate_one_dominates_iff,
      Multiset.sort_eq_replicate_iff] at hd
